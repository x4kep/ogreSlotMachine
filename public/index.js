...
class OgreSlotMachine{
  ...
  spin(e) {
      ...
      function displayReels(resultCombination, self) {
        resultCombination.forEach((reelResul) => {
          ...
          self.stopReels(addedReel, Number(reelResul), Default.reelsSpinRound, Default.Symbols);
        });
      }
  ...
  addReels(reelSymbols) {
    ...
    for(var i = 0; i <= Default.Symbols; i++){
    ...
    let blockTotalHeight = (Default.Symbols * blockRealHeight + (blockRealHeight));
